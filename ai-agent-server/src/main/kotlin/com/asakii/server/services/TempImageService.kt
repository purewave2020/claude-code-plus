...
object TempImageService {
    ...
    fun getImage(filename: String): File? {
        ...
        if (!file.exists() || !file.isFile) {
            logger.warn { "Temp image not found: $filename" }
            return null
        }
    ...
    fun getImageByPath(absolutePath: String): File? {
        ...
        if (!file.canonicalPath.startsWith(tempDir.canonicalPath)) {
            logger.warn { "Image path is outside temp directory: $absolutePath" }
            return null
        }

        if (!file.exists() || !file.isFile) {
            logger.warn { "Image not found: $absolutePath" }
            return null
        }
    ...
    private fun cleanupOldFiles() {
        try {
                ...
                if (file.isFile) {
                    ...
                    if (age > maxAge) {
                        ...
                        if (file.delete()) {
                            deletedCount++
                            deletedSize += size
                            logger.debug { "Deleted old temp image: ${file.name} (age: ${age / 1000 / 60 / 60}h)" }
                        }
                    }
                }
            }
            ...
            logger.error(e) { "Error during temp image cleanup" }
        }
    }
